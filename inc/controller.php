...
class Nexo_Controller extends CI_Model
{
	...
	function menus( $final )
	{
		...
		$menus[ 'clients' ]		=	$this->events->apply_filters( 'nexo_customers_menu_array', array(
			array( 
			...
			array( 
				'title'		=>	__( 'Campagne publicitaires', 'nexo' ),
				'href'		=>	'http://nexo.tendoo.org/get-premium', // site_url( 'dashboard/nexo/ads/lists' ),
			),
			array( 
				'title'		=>	__( 'Nouvelle campagne', 'nexo' ),
				'href'		=>	'http://nexo.tendoo.org/get-premium', // site_url( 'dashboard/nexo/ads/add' ),
			),
		) );
		
		$menus[ 'arrivages' ]	=	$this->events->apply_filters( 'nexo_shipping_menu_array', array(
			array( 
		...
		$final 	=	array_merge( $start, $menus, $end );
        return $final;	
	}
	...
	function load_controller()
	{
		...
		if( is_array( $this->args ) && count( $this->args ) > 0 ) {
			$file	=	dirname( __FILE__ ) . '/../controllers/' . $this->args[0] . '.php';
			if( is_file( $file ) ) {
				include_once( $file );
			} else {
