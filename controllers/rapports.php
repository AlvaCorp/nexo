...
class Nexo_Reports extends CI_Model
{
	...
	function journalier( $start_date = null, $end_date = null )
	{
		...
		$this->load->driver('cache', array('adapter' => 'file', 'backup' => 'file', 'key_prefix'	=>	'nexo_daily_reports_' ) );
		...
		if( get_instance()->cache->get( $data[ 'report_slug' ] ) && @$_GET[ 'refresh' ] != 'true' )
		{
			echo get_instance()->cache->get( $data[ 'report_slug' ] );
			return;
		}
