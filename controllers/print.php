...
class Nexo_Print extends CI_Model
{
	...
	function order_receipt( $order_id = null )
	{
		if( $order_id != null ) {
			...
			$this->load->driver('cache', array('adapter' => 'file', 'backup' => 'file', 'key_prefix'	=>	'nexo_order_' ));
			
			if( $order_cache = get_instance()->cache->get( $order_id ) && @$_GET[ 'refresh' ] != 'true' )
			{
	...
	**/
	...
	function shipping_item_codebar( $shipping_id = null )
	{
		...
		$this->load->driver('cache', array('adapter' => 'file', 'backup' => 'file', 'key_prefix'	=>	'nexo_products_labels_' ));
			
		if( $products_labels = get_instance()->cache->get( $shipping_id ) && @$_GET[ 'refresh' ] != 'true' )
		{
