	...
	this.CreateNewClientPopup			=	function(){
		bootbox.confirm( '<div id="NewClientPopup"/>', function( action ){
			if( action ) { // Si l'action vaut true
				...
				var content			=	_.object( [ 0, 1 ], [ customer_name, customer_email ] );
				$.ajax( '<?php echo site_url( array( 'dashboard', 'nexo', 'rest', 'post', 'model', 'Nexo_Misc', 'create_customer' ) );?>', {
					type:	"POST",
					data: 	content,
					dataType: "json",
					success: function( data ) {
						if( data.type == 'success' ) {
							...
							//  Refresh Client List
							$.ajax( '<?php echo site_url( array( 'dashboard', 'nexo', 'rest', 'get', 'nexo_clients', 'null' ) );?>', {
