...
if( ! $order_cache = get_instance()->cache->get( $order[ 'order' ][0][ 'ID' ] ) || @$_GET[ 'refresh' ] == 'true' )
{
	ob_start();
}
?>
...
<?php
if( ! get_instance()->cache->get( $order[ 'order' ][0][ 'ID' ] ) || @$_GET[ 'refresh' ] == 'true' )
{
	get_instance()->cache->save( $order[ 'order' ][0][ 'ID' ], ob_get_contents(), 999999999 ); // long time
}