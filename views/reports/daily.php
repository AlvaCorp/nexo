...
if( ! get_instance()->cache->get( $report_slug ) || @$_GET[ 'refresh' ] == 'true' )
{
	ob_start();
}
?>
...
<title><?php echo sprintf( __( 'Rapport des ventes journalières du %s au %s &mdash; Nexo', 'nexo' ), $CarbonStart->formatLocalized('%A %d %B %Y'), $CarbonEnd->formatLocalized('%A %d %B %Y') );?></title>
...
<script type="text/javascript" src="<?php echo module_url( 'nexo' );?>/bower_components/bootstrap/dist/js/bootstrap.min.js"></script>
...
<?php get_instance()->events->do_action( 'dashboard_header' );?>
</head>
	...
	<div class="container-fluid">
        <div class="hideOnPrint">
            ...
            <div class="btn-group" role="group" aria-label="...">
                <a href="<?php echo site_url( array( 'dashboard' ) );?>" class="btn btn-default">
                    <?php _e( 'Tableau de bord', 'nexo' );?>
                </a>
            </div>
            <div class="btn-group" role="group" aria-label="...">
                <a href="<?php echo current_url() . '?refresh=true';?>" class="btn btn-default">
                    <?php _e( 'Ignorer le cache', 'nexo' );?>
                </a>
            </div>
            <br>
            <br>
...
if( ! get_instance()->cache->get( $report_slug ) || @$_GET[ 'refresh' ] == 'true' )
{
	get_instance()->cache->save( $report_slug, ob_get_contents(), 999999999 ); // long time
}
	...
	**/
	...
	this.Start			=	function(){
		...
		$( '.get-reports' ).each( function(){
			Report.Dates.push( $(this).data( 'day' ) );
		});
			...
			success			:	function( json ){
				if( json.length > 0 ) {	
					...
					_.each( json, function( value, key ) {
						...
						if( _.contains( [ Report.CommandeCash, Report.CommandeDevis, Report.CommandeAvance ], value.TYPE ) ) {
							ChiffreDaffaireNet	+=	Math.abs( parseInt( value.TOTAL ) - RRR );
						}
					});
					$( '[data-day="' + tableItemId + '"]' )
						.find( '.total_chiffre_daffaire_net' )
						.html( Report.CurrencyBefore + ' ' + NexoAPI.Format( ChiffreDaffaireNet ) + ' ' + Report.CurrencyAfter );
					...
					$( '[data-day="' + tableItemId + '"]' )
						...
						.html( Report.CurrencyBefore + ' ' + NexoAPI.Format( CurrentTotal ) + ' ' + Report.CurrencyAfter );
					...
					_.each( json, function( value, key ) {
						...
						if( _.contains( [ Report.CommandeAvance, Report.CommandeDevis ], value.TYPE ) ) {
					...
					$( '[data-day="' + tableItemId + '"]' )
						...
						.html( Report.CurrencyBefore + ' ' + NexoAPI.Format( CurrentDues ) + ' ' + Report.CurrencyAfter );
					...
					$( '[data-day="' + tableItemId + '"]' )
						...
						.html( Report.CurrencyBefore + ' ' + NexoAPI.Format( CurrentRRR ) + ' ' + Report.CurrencyAfter );
...
<?php get_instance()->events->do_action( 'nexo_daily_report_footer', $CarbonStart, $CarbonEnd );?>
